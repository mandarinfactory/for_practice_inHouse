...
export const getBoxOfficeInfo = async (MovieInfosCtx) => {
  const url = `http://www.kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json?key=${import.meta.env.VITE_KOBIS_KEY}&targetDt=${
    date - 1
  }&itemPerPage=5`;
...
export const getSearchMovieInfo = async (MovieInfosCtx, setSearchedMovie) => {
  const url = `http://api.koreafilm.or.kr/openapi-data2/wisenut/search_api/search_json2.jsp?collection=kmdb_new2&ServiceKey=${import.meta.env.VITE_KMDB_KEY}&query=${MovieInfosCtx.searchMovieKeyword}`;
  const json = await fetchJson(url);
...
export const getSearchGenreMovies = async (
  ...
  const url = `http://api.koreafilm.or.kr/openapi-data2/wisenut/search_api/search_json2.jsp?collection=kmdb_new2&ServiceKey=${import.meta.env.VITE_KMDB_KEY}&genre=${genres}&releaseDts="20100101`;
  const json = await fetchJson(url);
